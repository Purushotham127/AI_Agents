...
SYSTEM_PROMPT = """You are a precise file management assistant.
You have access to tools to read, write, list, and delete files.
...
2. Always verify the JSON response status from tools before answering.
3. Do not output raw JSON blocks in your text response — use the tool interface."""
...
async def run_agent():
    ...
    try:
        ...
        # Create memory for last 5 interactions
        memory = ConversationBufferMemory(max_messages=5)

        agent = create_agent(
            model=llm,
            tools=tools,
            memory=memory,  # Added memory parameter
                    ...
                    if "model" in event:
                        ...
                        # ✅ Case 2: Model output raw JSON in content instead of tool_calls
                            ...
                            if raw_call:
                                ...
                                print(f"\n✅  [Fallback: detected raw tool call for '{tool_name}']")
